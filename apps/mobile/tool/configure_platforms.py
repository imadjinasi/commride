...
def configure_android() -> None:
    ...
    for gradle_name in ("build.gradle.kts", "build.gradle"):
        ...
        if count != 1:
            raise SystemExit("Unexpected Android applicationId template; refusing unsafe patch.")
...
def configure_ios() -> None:
    ...
    setup = "    FLTFirebaseMessagingPlugin.configureNotificationCenterDelegate()\n"
    if "configureNotificationCenterDelegate()" not in content:
        ...
        if marker not in content:
            raise SystemExit("Unexpected iOS delegate template; refusing unsafe patch.")
